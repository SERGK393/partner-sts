...
foreach (array("DEFAULT", "OTHER") as $type)
{
	foreach ($arResult["SET_ITEMS"][$type] as $key=>$arItem)
	{
		$arElement = array(
            ...
            "PURCHASE_PRICE_VALUE" => $arItem["PURCHASE_PRICE_VALUE"],
            "PURCHASE_PRICE_PRINT_VALUE" => $arItem["PURCHASE_PRICE_PRINT_VALUE"],
            "RETAIL_PRICE_VALUE" => $arItem["RETAIL_PRICE_VALUE"],
		);
		...
		if ($arItem['DETAIL_PICTURE'] || $arItem['PREVIEW_PICTURE'])
		{
			$arFileTmp = CFile::ResizeImageGet(
				...
				array("width" => "140", "height" => "60"),
				BX_RESIZE_IMAGE_PROPORTIONAL,
				true
			);
			$arElement['PREVIEW_PICTURE'] = $arFileTmp;
		}
...
$arResult["SET_ITEMS"]['ALL']=array_merge($arResult["SET_ITEMS"]['DEFAULT'],$arResult["SET_ITEMS"]['OTHER']);
foreach($arResult["SET_ITEMS"]['ALL'] as $key=>$item){
    $group='';
    preg_match('#vanna-(.+?)-#',$item['DETAIL_PAGE_URL'],$group);
    if($group[1])$arResult["SET_ITEMS"]['ALL'][$key]['GROUP']=$group[1];
}
