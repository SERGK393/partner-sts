...
if($this->StartResultCache(false, array($elementID, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()))))
{
    ...
    $_PARTNER_ID = isPartnerClient();
    $arResult['PARTNER_ID']=$_PARTNER_ID;
	...
	while ($obElement = $dbElement->GetNextElement())
	{
		...
		{
            $arItem['PURCHASE_PRICE_VALUE'] = 0;
            $arItem['PURCHASE_PRICE_PRINT_VALUE'] = '';
            $arItem['RETAIL_PRICE_VALUE'] = 0;
			...
			$minPrice = 0;
			foreach($arPrices as $_PRICE_CODE=>$arPrice)
			{
                if($_PARTNER_ID) {
                    if($_PRICE_CODE == 'purchasePrice_'.$_PARTNER_ID) {
                        if($arPrice["VALUE"]) {
                            $arItem['PURCHASE_PRICE_VALUE'] = $arPrice["VALUE"];
                            $arItem['PURCHASE_PRICE_PRINT_VALUE'] = $arPrice["PRINT_VALUE"];
                        }
                    }
                    if($_PRICE_CODE == 'retailPrice_'.$_PARTNER_ID)
                        if($arPrice["VALUE"]) $arItem['RETAIL_PRICE_VALUE'] = $arPrice["VALUE"];
                }
                if ($_PRICE_CODE == 'BASE')
				//if ($arPrice['MIN_PRICE'] == "Y")
				{
					...
					if(!$arItem['PURCHASE_PRICE_VALUE']) {
                        if(!$_PARTNER_ID)$arItem['PURCHASE_PRICE_VALUE'] = $arPrice["VALUE"];
                        $arItem['PURCHASE_PRICE_PRINT_VALUE'] = $arPrice["PRINT_VALUE"];
                    }
                    if(!$arItem['RETAIL_PRICE_VALUE']) $arItem['RETAIL_PRICE_VALUE'] = $arPrice["VALUE"];
