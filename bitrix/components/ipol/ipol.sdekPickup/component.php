...
if($arReturn['DELIVERY']['pickup'] != 'no'){
	$arList = CDeliverySDEK::wegihtPVZ((CDeliverySDEK::$orderWeight)?false:COption::GetOptionString(CDeliverySDEK::$MODULE_ID,'weightD',1000));
	foreach($arList as $city => $PVZs){
		if($allCities || in_array($city,$arParams['CITIES'])){
			$arResult['PVZ'][$city] = $PVZs;
