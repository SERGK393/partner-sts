...
class CSaleYMHandler1
{
	...
	public function __construct($arParams = array())
	{
		...
		else */$this->mapDelivery['62']='DELIVERY';
	...
	private function checkCityOutlet($cityName){
	    $result = false;
	    if (CModule::IncludeModule("iblock")) {
            $arOrder = Array("SORT"=>"ASC");
            $arFilter = Array('IBLOCK_ID'=>18,"NAME"=>$cityName);
            $select = array('ID');
            $res = CIBlockSection::GetList($arOrder,$arFilter,false,$select);
            $res = $res->getNext();
            $id = intval($res['ID']);
            
            if(!empty($id))$result=true;
        }
        return $result;
	}
	
	private function getOutletsByCity($cityName){
	    $arResult = array();
	    if (CModule::IncludeModule("iblock")) {
            $arOrder = Array("SORT"=>"ASC");
            $arFilter = Array('IBLOCK_ID'=>18,"NAME"=>$cityName);
            ...
            foreach($this->outlets as $outlet){
                if(intval($outlet/100)==$id){
                    $arResult[]=intval($outlet);
                }
            }
        }
        return $arResult;
	...
	private function _GetPrice($city,$idOrd,$morz,$cur=false){
        if (CModule::IncludeModule("iblock")) {
            $arOrder = Array("SORT"=>"ASC");
            $arFilter = Array('IBLOCK_ID'=>18,"NAME"=>$city);
            ...
            $select = Array('UF_LOGISTYC_INDEX','XML_ID');
...
///////////////////////////////////////////////////////////////
	...
	protected function getDeliveryOptions($delivery, $totalPrice, $arItems, $weight = 0)
	{
		/*$arResult = $this->mapDelivery;
		...
		$dates=array(
		    "fromDate"=> date('d-m-Y'),
		    "toDate"=> date('d-m-Y',time()+(60*60*24))
	    );
		
		if ($delivery["region"]["name"]=='Воронеж'){
		    $arDeliveryTmp = array(
			    "id" => '62',
			    "type" =>'PICKUP',
			    "serviceName" => 'Доставка до пункта выдачи',
			    "price" => 0,
			    "dates" => $dates
		    );
		    
		    $outlets=$this->getOutletsByCity('Воронеж');
		    if(!empty($outlets)){
			...
			if($totalPrice < 4000){
                foreach($arItems as $prId){
                    ...
                    $morz=$res['VALUE'];
                    if($price!=0){
                        $price+=$this->_GetPrice('Воронеж',$prId,$morz,$cur=true)*0.1;
                    }else{
                        $price+=$this->_GetPrice('Воронеж',$prId,$morz);
                    }
                }
            }
            //$price=0;
		    ...
		    if($totalPrice > 29999){
                ...
                foreach($arItems as $prId){
                    ...
                    $morz=$res['VALUE'];
                    if($price!=0){
                        $price+=$this->_GetPrice($delivery["region"]["name"],$prId,$morz)*0.1;
                    }else{
                        $price+=$this->_GetPrice($delivery["region"]["name"],$prId,$morz);
                    }
                }
            }
            //$price=$totalPrice;
	        ...
	        $outlets=$this->getOutletsByCity($delivery["region"]["name"]);
	        if(!empty($outlets)){
	            foreach($outlets as $outlet)
			        $arDeliveryTmp["outlets"][] = array("id" => intval($outlet));
            ...
            if($totalPrice > 49999){
                ...
                $region='Москва';
                if($this->checkCityOutlet($delivery["region"]["name"]))
                    $region=$delivery["region"]["name"];
                foreach($arItems as $prId){
                    $res=CIBlockElement::GetProperty(10,$prId,array(),array("CODE"=>"morz"));
                    $res=$res->GetNext();
                    $morz=$res['VALUE'];
                    if($price!=0){
                        $price+=$this->_GetPrice($region,$prId,$morz)*0.1;
                    }else{
                        $price+=$this->_GetPrice($region,$prId,$morz,$cur=true);
                    }
                }
            };
            //$price=0;
	...
	protected function processCartRequest($arPostData)
	{
		...
		if( $this->checkCartStructure($arPostData))
		{
			...
			foreach ($arPostData["cart"]["items"] as $arItem)
			{
				...
				elseif(!empty($item))
				{
				    if($item["count"]!=null){
					    ...
					    $arItems[''.$item['l_index'].'_'.$item['offerId']]=$item['offerId'];
			...
			{
				$arResult["cart"]["deliveryOptions"] = $this->getDeliveryOptions($arPostData["cart"]["delivery"],/*$cartPrice*/$totalPrice,$arItems,$cartWeight);
				//$arResult["set"] = $this->getSettingsBySiteId($this->siteId);
	...
	private function getAddressByData($arPostData){
	    if (CModule::IncludeModule("iblock")&&!isset($arPostData["order"]["delivery"]["address"])){
	        $outletId = $arPostData["order"]["delivery"]["outlet"]["id"];
	        if(!empty($outletId)){
	            $sectionId = intval($outletId/100);
	            $outletNum = intval($outletId)%100;
	            
                $arOrder = Array("SORT"=>"ASC");
                $arFilter = Array('IBLOCK_ID'=>18,"SECTION_ID"=>$sectionId,"SORT"=>$outletNum);
                $select = array('NAME');
                $res = CIBlockElement::GetList($arOrder,$arFilter,false,$select);
                $res = $res->getNext();
                $address = $res['NAME'];
	            
	            return array(
		            "country"=>"Россия",
		            "city"=>$arPostData["order"]["delivery"]["region"]["name"],
		            "street"=>$address,
		            "house"=>""
		        );  
		    }
		}
	    return $arPostData["order"]["delivery"]["address"];
	}
	...
	protected function processOrderAcceptRequest($arPostData)
	{
		...
		if( $this->checkOrderAcceptStructure($arPostData))
		{
			...
			if(!$orderCorrespondence = $dbRes->fetch())
			{
				...
				$arOptions = array();
				$arAddress=$this->getAddressByData($arPostData);
				...
				$arOrder['DELIVERY_PRICE']=intval($arPostData["order"]["delivery"]["price"]);//additional
