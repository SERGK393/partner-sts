...
if (($ACTION == 'EXPORT_EDIT' || $ACTION == 'EXPORT_COPY') && $STEP == 1)
{
	...
	if (array_key_exists('V', $arOldSetupVars))
		$V = $arOldSetupVars['V'];
	if (array_key_exists('vendors_deny', $arOldSetupVars))
		$vendors_deny = $arOldSetupVars['vendors_deny'];
...
<tr>
	<td width="40%" valign="top">Выберите бренды для их исключения:</td>
	<td width="60%">
	<?
	if(CModule::IncludeModule("highloadblock")){
        ...
        ?>
        <select class="adm-select-multiple" id="vendors_deny" name="vendors_deny[]" multiple="" size="15">
        <?while($res = $result->fetch()){?>
            <option value="<?=$res['UF_XML_ID']?>"<?=in_array($res['UF_XML_ID'],$vendors_deny)?' selected':''?>><?=$res['UF_NAME']?></option>
        <?}?>
        </select>
        <?
	}
...
if (2 > $STEP)
{
	...
	<input type="hidden" name="SETUP_FIELDS_LIST" value="V,vendors_deny,IBLOCK_ID,SETUP_SERVER_NAME,SETUP_FILE_NAME,XML_DATA">
