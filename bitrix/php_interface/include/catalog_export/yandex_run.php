...
$arFilter = Array('IBLOCK_ID'=>18,"NAME"=>'Воронеж');
$select=array('UF_LI1_PRICE','UF_LI2_PRICE','UF_LI3_PRICE','UF_LI4_PRICE');
$res=CIBlockSection::GetList($arOrder ,$arFilter,false,$select);
$res=$res->getNext();
$arDeliveryLogisticIndex=array(1=>$res['UF_LI1_PRICE'],$res['UF_LI2_PRICE'],$res['UF_LI3_PRICE'],$res['UF_LI4_PRICE']);*/
$arDeliveryLogisticIndex=array(1=>200,500,500,500);
$price_full_delivery=6000;

$arYandexFields = array('vendor', 'vendorCode', 'model', 'author', 'name', 'store', 'pickup', 'delivery', 'local_delivery_cost', 'publisher', 'series', 'year', 'ISBN', 'volume', 'part', 'language', 'binding', 'page_extent', 'table_of_contents', 'performed_by', 'performance_type', 'storage', 'format', 'recording_length', 'artist', 'title', 'year', 'media', 'starring', 'director', 'originalName', 'country', 'aliases', 'description', 'sales_notes', 'promo', 'provider', 'tarifplan', 'xCategory', 'additional', 'worldRegion', 'region', 'days', 'dataTour', 'hotel_stars', 'room', 'meal', 'included', 'transport', 'price_min', 'price_max', 'options', 'manufacturer_warranty', 'country_of_origin', 'downloadable', 'param', 'place', 'hall', 'hall_part', 'is_premiere', 'is_kids', 'date',);
...
if (empty($arRunErrors))
{
	CheckDirPath($_SERVER["DOCUMENT_ROOT"].$SETUP_FILE_NAME.'.tmp');

	if (!$fp = @fopen($_SERVER["DOCUMENT_ROOT"].$SETUP_FILE_NAME.'.tmp', "wb"))
	{
		$arRunErrors[] = str_replace('#FILE#', $_SERVER["DOCUMENT_ROOT"].$SETUP_FILE_NAME.'.tmp', GetMessage('YANDEX_ERR_FILE_OPEN_WRITING'));
	}
	else
	{
		...
		{
			$arRunErrors[] = str_replace('#FILE#', $_SERVER["DOCUMENT_ROOT"].$SETUP_FILE_NAME.'.tmp', GetMessage('YANDEX_ERR_SETUP_FILE_WRITE'));
			@fclose($fp);
		}
		else
		{
...
/*					$dbVAT = CCatalogProduct::GetVATInfo($arAcc['ID']);
			...
			$strTmpOff_cancel = false;
			$strTmpOff_old = $strTmpOff;
			...
			foreach ($arYandexFields as $key)
			{
				...
				case 'param':
					...
					break;
				case 'delivery':
				    $strTmpOff.= "<".$key.">true</".$key.">\n";
				    break;
				case 'local_delivery_cost':
				    $ldc_id= $arAcc["~ID"];//Product ID
                    $ldc_arSelect=array('ID');
                    $ldc_res=CIBlockElement::GetElementGroups(
                    $ldc_id,false,
                    $ldc_arSelect
                    );
                    $ldc_res=$ldc_res->getNext();
                    $ldc_sId=$ldc_res['ID'];
                    ////////////////////////////////////////////////////////////////
                    $ldc_arOrder = Array("SORT"=>"ASC");
                    $ldc_arFilter = Array('IBLOCK_ID'=>$arAcc["~IBLOCK_ID"],'ID'=>$ldc_sId);//IBLOCK_ID
                    $ldc_select = Array('UF_LOGISTYC_INDEX');

                    $ldc_res=CIBlockSection::GetList(
                        $ldc_arOrder,
                        $ldc_arFilter,false,
                        $ldc_select,false
                    );
                    ...
                    $ldc_price=$arDeliveryLogisticIndex[$ldc_res['UF_LOGISTYC_INDEX']];//ARRESULT
                    if($minPrice>=$price_full_delivery)$ldc_price=0;

                    $strTmpOff.= "<".$key.">".yandex_text2xml($ldc_price, true)."</".$key.">\n";//writing to yml
				    break;
				case 'model':
				case 'title':
				...
				default:
					if (is_array($XML_DATA) && is_array($XML_DATA['XML_DATA']) && $XML_DATA['XML_DATA'][$key])
					{
					    if($key == 'vendor') if(in_array($arAcc['PROPERTIES'][$key]['VALUE'],$vendors_deny)) {
					        $strTmpOff_cancel = true;
					        break;
					    }
				...
				if (!empty($arCurrentOffer) && !empty($arCurrentPrice))
				{
					...
					foreach ($arYandexFields as $key)
					{
						...
						case 'param':
							...
							break;
				        case 'delivery':
				            $strOfferYandex.= "<".$key.">true</".$key.">\n";
				            break;
				        case 'local_delivery_cost':
				            $ldc_id= $arOfferItem["~ID"];//Product ID
                            $ldc_arSelect=array('ID');
                            $ldc_res=CIBlockElement::GetElementGroups(
                            $ldc_id,false,
                            $ldc_arSelect
                            ...
                            ////////////////////////////////////////////////////////////////
                            $ldc_arOrder = Array("SORT"=>"ASC");
                            $ldc_arFilter = Array('IBLOCK_ID'=>$arOfferItem["~IBLOCK_ID"],'ID'=>$ldc_sId);//IBLOCK_ID
                            $ldc_select = Array('UF_LOGISTYC_INDEX');
                            ...
                            $strOfferYandex.= "<".$key.">".yandex_text2xml($ldc_price, true)."</".$key.">\n";//writing to yml
				            break;
						case 'model':
						case 'title':
							if (!is_array($XML_DATA) || !is_array($XML_DATA['XML_DATA']) || !$XML_DATA['XML_DATA'][$key])
					...
					// no break here
						default:
							if (is_array($XML_DATA) && is_array($XML_DATA['XML_DATA']) && $XML_DATA['XML_DATA'][$key])
							{
							    if($key == 'vendor') if(in_array($arOfferItem['PROPERTIES'][$key]['VALUE'],$vendors_deny)) {
					                $strOfferYandex_cancel = true;
					                break;
					            }
								$strValue = '';
								$strValue = yandex_get_value($arOfferItem, $key, $XML_DATA['XML_DATA'][$key], $arProperties, $arUserTypeFormat);
					...
					if($strOfferYandex_cancel) continue;
...
/*						$dbVAT = CCatalogProduct::GetVATInfo($arItem['ID']);
				...
				foreach ($arYandexFields as $key)
				{
					...
					case 'param':
						...
						break;
			        case 'delivery':
						$strValue = "<".$key.">true</".$key.">\n";
						break;
			        case 'local_delivery_cost':
			            $ldc_id= $arItem["~ID"];//Product ID
                        $ldc_arSelect=array('ID');
                        $ldc_res=CIBlockElement::GetElementGroups(
                        $ldc_id,false,
                        $ldc_arSelect
                        ...
                        ////////////////////////////////////////////////////////////////
                        $ldc_arOrder = Array("SORT"=>"ASC");
                        $ldc_arFilter = Array('IBLOCK_ID'=>$arItem["~IBLOCK_ID"],'ID'=>$ldc_sId);//IBLOCK_ID
                        $ldc_select = Array('UF_LOGISTYC_INDEX');
                        ...
                        $strValue = "<".$key.">".yandex_text2xml($ldc_price, true)."</".$key.">\n";//writing to yml
			            break;
					case 'model':
					case 'title':
						if (!is_array($XML_DATA) || !is_array($XML_DATA['XML_DATA']) || !$XML_DATA['XML_DATA'][$key])
					...
					default:
						...
						{
						    if($key == 'vendor') if(in_array($arItem['PROPERTIES'][$key]['VALUE'],$vendors_deny)) {
				                $strOfferYandex_cancel = true;
				                break;
				            }
							$strValue = yandex_get_value($arItem, $key, $XML_DATA['XML_DATA'][$key], $arProperties, $arUserTypeFormat);
    ...
    copy($_SERVER["DOCUMENT_ROOT"].$SETUP_FILE_NAME.'.tmp', $_SERVER["DOCUMENT_ROOT"].$SETUP_FILE_NAME);
