...
<?
        $arResult['JS_DATA']['EXTRA']['AJAX_PATH'] = $templateFolder;
        $rsUser = $USER->GetByID($USER->GetID());
        $arUser = $rsUser->Fetch();
        $entity = $arUser['UF_PARTNER_XML_ID'];
        if($entity){
        	$arResult['JS_DATA']['EXTRA']['PARTNER_XML_ID'] = $entity;
        	if(CModule::IncludeModule("highloadblock")){
                ...
                if($res = $result->fetch()){
                    $arResult['JS_DATA']['EXTRA']['PARTNER_ID'] = $arResult['JS_DATA']['EXTRA']['PARTNER_XML_ID'];
                	$arResult['JS_DATA']['EXTRA']['PARTNER_ID_1'] = $res['UF_ALT_XML_ID'];
                    $arResult['JS_DATA']['EXTRA']['PARTNER_ID_2'] = $res['UF_ALT_XML_ID_1'];
                    $arResult['JS_DATA']['EXTRA']['PARTNER_ID_3'] = $res['UF_ALT_XML_ID_2'];

                    foreach($arResult['JS_DATA']['ORDER_PROP']['properties'] as &$val){
                    	if(empty($val['VALUE'][0])||$val['IS_ZIP']=='Y')
                        switch ($val['CODE']){
                            case 'COMPANY':
                            $val['VALUE'][0] = $res['UF_COMPANY_NAME'];
                            break;
                            case 'COMPANY_ADR':
                            if(!empty($res['UF_JUR_ADDRESS'])) $val['VALUE'][0] = $res['UF_JUR_ADDRESS'];
                            break;
                            case 'INN':
                            $val['VALUE'][0] = $res['UF_INN'];
                            break;
                            case 'ZIP':
                            if(!empty($res['UF_INDEX'])) $val['VALUE'][0] = $res['UF_INDEX'];
                            break;
                            case 'PHONE':
                            if(!empty($res['UF_PHONE'])) $val['VALUE'][0] = $res['UF_PHONE'];
                            break;
                            case 'CITY':
                            $val['VALUE'][0] = $res['UF_CITY'];
                            break;
                            case 'ADDRESS':
                            if(!empty($res['UF_JUR_ADDRESS'])) $val['VALUE'][0] = $res['UF_JUR_ADDRESS'];
                            break;
...
?>
		...
		<input type="hidden" name="ORDER_PROP_72" value="<?=$arResult['JS_DATA']['EXTRA']['PARTNER_ID']?>">
