	...
	BX.Sale.OrderAjaxComponent = {
		...
		initOptions: function()
		{
			...
			if (this.result.TOTAL)
			{
				...
				this.options.showOrderWeight = false;//total.ORDER_WEIGHT && parseFloat(total.ORDER_WEIGHT) > 0;
				this.options.showPriceWithoutDiscount = false;//parseFloat(total.ORDER_PRICE) < parseFloat(total.PRICE_WITHOUT_DISCOUNT_VALUE);
				this.options.showDiscountPrice = false;//total.DISCOUNT_PRICE && parseFloat(total.DISCOUNT_PRICE) > 0;
				this.options.showTaxList = false;//total.TAX_LIST && total.TAX_LIST.length;
				this.options.showPayedFromInnerBudget = false;//total.PAYED_FROM_ACCOUNT_FORMATED && total.PAYED_FROM_ACCOUNT_FORMATED.length;
			}
		},
		...
		clickOrderSaveAction: function(event)
		{
			if (this.isValidForm())
			{
				...
				var cart_names = {};
				var cart_prices = {};
				var partner_id = this.result.EXTRA.PARTNER_ID;
                ...
                if(payment_24) alt_partner_id = this.result.EXTRA.PARTNER_ID_1;
                if(payment_25) alt_partner_id = this.result.EXTRA.PARTNER_ID_2;
                if(payment_26) alt_partner_id = this.result.EXTRA.PARTNER_ID_3;
				var comment = '';
				console.log([payment_24,payment_25,payment_26,alt_partner_id,partner_id]);

				if(partner_id){
					$('.bx-soa-item-table>.bx-soa-basket-info').each(function(){
						var sku=$(this).data('xml-id');
						var count=$(this).data('quantity');
						var price=$(this).data('price');
						var name=$(this).find('.bx-soa-item-title a').text();
						//alert(sku+' '+name+' '+count);
						cart_content[sku]=count;
						cart_prices[sku]=price;
						cart_names[sku]=name;
					});
					comment = $('#orderDescription').val();
					...
					$.post( this.result.EXTRA.AJAX_PATH + "/ajax.php", objSend ,function( msg ) {
						if(msg.status == 'ok'){
							if(msg.response.STRING == 'NOT_FOUND'){
								obj.modalAlert(msg.response.COMMENT,{
                                    name:"Да, добавить позиции, отсутствующие на складе, в заявку(N/A)",
                                    callback:function(){}
                                },{
								...
								//if(confirm(msg.response.COMMENT)){
                                    ...
                                    $('input[name="ORDER_PROP_72"]').attr('value',alt_partner_id);

									obj.allowOrderSave();

									if (obj.params.USER_CONSENT === 'Y')
									{
		...
		createBasketItem: function(basketItemsNode, item, index, active)
		{
			...
			basketItemsNode.appendChild(
				BX.create('DIV', {
					...
					attrs: {'data-xml-id':item.data.PRODUCT_XML_ID, 'data-quantity':item.data.QUANTITY, 'data-price':item.data.PRICE},
					children: cols
				})
			);
		...
		getPersonTypeControl: function(node)
		{
			...
			else if (personTypesCount == 2)
			{
				for (i in this.result.PERSON_TYPE)
				{
					if (this.result.PERSON_TYPE.hasOwnProperty(i) && (this.result.PERSON_TYPE[i].CHECKED == 'Y' || !(this.result.PARTNER_XML_ID)))
					{
						currentType = this.result.PERSON_TYPE[i];
						label = BX.create('LABEL', {
							children: [
		...
		{
			if(property.getId() == 72 || property.getId() == 73) return;
		...
		alterProperty: function(settings, propContainer)
		{
			...
			if (textNode)
			{
				...
				if(textNode.getAttribute("rows")>5)
