...
window.JCCatalogSection = function (arParams)
{
	...
	if ('object' === typeof arParams)
	{
		...
		if (!!arParams.BASKET && 'object' === typeof(arParams.BASKET))
		{
			...
			if (!!arParams.BASKET.XML_ID)
			{
				this.basketData.xml_id = arParams.BASKET.XML_ID;
			}
            if (!!arParams.BASKET.NAME)
            {
                this.basketData.name = arParams.BASKET.NAME;
            }
            ...
            if (!!arParams.PARTNER_ID)
            {
                this.partnerID = arParams.PARTNER_ID;
            }
...
window.JCCatalogSection.prototype.SendToBasket = function()
{
    ...
    var partnerButton = $('.toOrder.bb'+this.basketData.xml_id);
    if(partnerButton.hasClass('active') || !this.partnerID){
...
window.setProductNameSku = function (productName,productSku,partnerID){
...
window.JCCatalogSection.prototype.Basket = function()
{
    ...
    var partnerButton = $('.toOrder.bb'+obj.basketData.xml_id);
    if(partnerButton.hasClass('active') || !obj.partnerID) {
        if (!partnerButton.hasClass('succes')) {
            partnerButton.removeClass('active');
            partnerButton.html('<svg width="50px"  height="50px"  xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" preserveAspectRatio="xMidYMid" class="lds-rolling" style="background: none;">' +
                '<circle cx="50" cy="50" fill="none" ng-attr-stroke="{{config.color}}" ng-attr-stroke-width="{{config.width}}" ng-attr-r="{{config.radius}}" ng-attr-stroke-dasharray="{{config.dasharray}}" stroke="#ffb175" stroke-width="12" r="18" stroke-dasharray="84.82300164692441 30.274333882308138" transform="rotate(186 50 50)">' +
            ...
            if (obj.templatePath) {
                var cart_content = {};
                cart_content[obj.basketData.xml_id] = "1";
                $.post(obj.templatePath + "/ajax.php", {"cart_content": cart_content}, function (msg) {
                    partnerButton.addClass('active');

                    if (msg.status == 'ok') {
                        if (msg.response.STRING == 'NOT_FOUND') {
                            partnerButton.text('Уточнить');
                            window.setProductNameSku(obj.basketData.name,obj.basketData.xml_id,obj.partnerID);
                            return;
                        }
                    } else {
                        partnerButton.text('Ошибка');
                        return;
                    }
                    ...
                    switch (obj.productType) {
                        case 1://product
                        case 2://set
                            if (obj.basketData.useProps && !obj.basketData.emptyProps) {
                                obj.InitPopupWindow();
                                obj.obPopupWin.setTitleBar({
                                    content: BX.create('div', {
                                        style: {marginRight: '30px', whiteSpace: 'nowrap'},
                                        text: BX.message('TITLE_BASKET_PROPS')
                                    })
                                });
                                if (BX(obj.visual.BASKET_PROP_DIV)) {
                                    contentBasketProps = BX(obj.visual.BASKET_PROP_DIV).innerHTML;
                                }
                                obj.obPopupWin.setContent(contentBasketProps);
                                obj.obPopupWin.setButtons([
                                    new BasketButton({
                                        ownerClass: obj.obProduct.parentNode.parentNode.className,
                                        text: BX.message('BTN_MESSAGE_SEND_PROPS'),
                                        events: {
                                            click: BX.delegate(obj.SendToBasket, obj)
                                        }
                                    })
                                ]);
                                obj.obPopupWin.show();
...
window.JCCatalogSection.prototype.BasketResult = function(arResult)
{
    ...
    if(this.partnerID){
        ...
        if ('OK' === arResult.STATUS){
            partnerButton.addClass('succes').text('В заявке →').wrap('<a href="'+(!!this.basketData.basketUrl ? this.basketData.basketUrl : BX.message('BASKET_URL'))+'"></a>');
        }else{
            partnerButton.text('Ошибка');
            partnerButton.addClass('active');
        }
