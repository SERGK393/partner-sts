...
$arResult["SORTING_STRING"]=sprintf('<div class="nonebx_catalog_sort"><span>Сортировать по: </span>
<a href="%s" class="'.$arParams["ELEMENT_SORT_CUSTOM_catalog_PRICE_1"].'">цене</a> |
<a href="%s" class="'.$arParams["ELEMENT_SORT_CUSTOM_property_vendor"].'">производителю</a></div>',
$APPLICATION->GetCurPageParam("sort=catalog_PRICE_1",array("sort"), false),
$APPLICATION->GetCurPageParam("sort=property_vendor",array("sort"), false)
);
...
if (!empty($arResult['ITEMS']))
{
	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
        ...
        if($_SESSION['TF_LOCATION_SELECTED_CITY_NAME']=='Краснодар') {
            $db_props = CIBlockElement::GetProperty($arParams['IBLOCK_ID'], $arItem['ID'], array("sort" => "asc"), Array("CODE"=>"vendor"));
            if($ar_props = $db_props->Fetch())
                $vendor = $ar_props["VALUE"];
            else $vendor = '';
            if($vendor=='Акватон'){
                $arItem['PRICES']['BASE']['VALUE']=$arItem['PRICES']['BASE']['VALUE']*1.07;//Краснодар
                $arItem['PRICES']['BASE']['PRINT_DISCOUNT_VALUE_VAT']=CurrencyFormat($arItem['PRICES']['BASE']['VALUE'], 'RUB');
            }
        }
        
        /*if(isset($arItem['PRICES']['SPEC'])){
            $arItem['MIN_PRICE_EXTRA']['VALUE']=$arItem['PRICES']['BASE']['VALUE'];
            $arItem['MIN_PRICE_EXTRA']['PRINT_DISCOUNT_VALUE_VAT']=$arItem['PRICES']['BASE']['PRINT_DISCOUNT_VALUE_VAT'];
        }else{*/
            $arItem['MIN_PRICE_EXTRA']['VALUE']=$arItem['PRICES']['BASE']['DISCOUNT_VALUE'];
            $arItem['MIN_PRICE_EXTRA']['PRINT_DISCOUNT_VALUE_VAT']=$arItem['PRICES']['BASE']['PRINT_DISCOUNT_VALUE'];
			$arItem['MIN_PRICE_RETAIL']['VALUE']=$arItem['PRICES']['BASE']['VALUE'];
			$arItem['MIN_PRICE_RETAIL']['PRINT_DISCOUNT_VALUE_VAT']=$arItem['PRICES']['BASE']['PRINT_DISCOUNT_VALUE'];
        //}
        $partnerId = isPartnerClient();
		$arResult['PARTNER_ID'] = $partnerId;
		if($partnerId){
			$arPrice = CCatalogIBlockParameters::getPriceTypesList();
		    if(isset($arPrice['purchasePrice_'.$partnerId])){
		        $arItem['MIN_PRICE_EXTRA']['VALUE']=$arItem['PRICES']['purchasePrice_'.$partnerId]['VALUE'];
		        $arItem['MIN_PRICE_EXTRA']['PRINT_DISCOUNT_VALUE_VAT']=$arItem['PRICES']['purchasePrice_'.$partnerId]['PRINT_VALUE'];
		    }
		    if(isset($arPrice['retailPrice_'.$partnerId])){
		        $arItem['MIN_PRICE_RETAIL']['VALUE']=$arItem['PRICES']['retailPrice_'.$partnerId]['VALUE'];
		        $arItem['MIN_PRICE_RETAIL']['PRINT_DISCOUNT_VALUE_VAT']=$arItem['PRICES']['retailPrice_'.$partnerId]['PRINT_VALUE'];
		    }
		}/*elseif(count($_SESSION["SESS_AUTH"]["GROUPS"])){
            $key_extra_price='';
            $key_extra_price_okr='';
            foreach($arItem['PRICES'] as $price_key=>$price_array){
            ...
            if(!empty($key_extra_price)){
                $arItem['MIN_PRICE_EXTRA']['VALUE']=$arItem['PRICES'][$key_extra_price]['VALUE'];
                $arItem['MIN_PRICE_EXTRA']['PRINT_DISCOUNT_VALUE_VAT']=$arItem['PRICES'][$key_extra_price]['PRINT_DISCOUNT_VALUE_VAT'];
            }
        }*/
		...
		$arItem['NALICH']=$arItem['CATALOG_QUANTITY']>0;
