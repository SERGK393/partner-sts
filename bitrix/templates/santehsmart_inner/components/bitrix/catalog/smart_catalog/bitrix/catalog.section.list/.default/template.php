...
?></div><?
}elseif($arResult['SECTION']['DEPTH_LEVEL']>1){?>
...
$prnt=$arResult['SECTION']['IBLOCK_SECTION_ID'];

  $arFilter = Array('SECTION_ID'=>$prnt, 'ACTIVE'=>'Y');
  $db_list = CIBlockSection::GetList(Array('SORT'=>'ASC'), $arFilter, true);
  while($ar_result = $db_list->GetNext())
  {
	  $stl=($ar_result['ID']==$arResult['SECTION']['ID'])? "id='catalog__selected-item'":"";?>
	<li id="<?=$this->GetEditAreaId($ar_result['ID']);?>">
		<h2 class="bx_sitemap_li_title"><a href="<?=$ar_result['SECTION_PAGE_URL']?>" <?=$stl?>><?=$ar_result['NAME']?> <span style="font-size: 0,7em">(<?=$ar_result['ELEMENT_CNT']?>)</span></a></h2>
	</li>
