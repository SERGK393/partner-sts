...
$arResult['NALICH']=$arResult['CATALOG_QUANTITY']>0;

if($_SESSION['TF_LOCATION_SELECTED_CITY_NAME']=='Краснодар') {
    $db_props = CIBlockElement::GetProperty($arParams['IBLOCK_ID'], $arResult['ID'], array("sort" => "asc"), Array("CODE"=>"vendor"));
    if($ar_props = $db_props->Fetch())
        $vendor = $ar_props["VALUE"];
    else $vendor = '';
    if($vendor=='Акватон'){
        $arResult['PRICES']['BASE']['VALUE']=$arResult['PRICES']['BASE']['VALUE']*1.07;//Краснодар
        $arResult['PRICES']['BASE']['PRINT_DISCOUNT_VALUE_VAT']=CurrencyFormat($arResult['PRICES']['BASE']['VALUE'], 'RUB');
        $arResult['MIN_PRICE']['VALUE']=$arResult['PRICES']['BASE']['VALUE'];
        $arResult['MIN_PRICE']['PRINT_DISCOUNT_VALUE_VAT']=$arResult['PRICES']['BASE']['PRINT_DISCOUNT_VALUE_VAT'];
        $arResult['MIN_PRICE']['PRINT_DISCOUNT_VALUE']=$arResult['PRICES']['BASE']['PRINT_DISCOUNT_VALUE_VAT'];
    }
}

$arResult['MIN_PRICE_EXTRA']['VALUE']=$arResult['PRICES']['BASE']['VALUE'];
$arResult['MIN_PRICE_EXTRA']['PRINT_DISCOUNT_VALUE_VAT']=$arResult['PRICES']['BASE']['PRINT_DISCOUNT_VALUE'];
$arResult['MIN_PRICE_RETAIL']['VALUE']=$arResult['PRICES']['BASE']['VALUE'];
$arResult['MIN_PRICE_RETAIL']['PRINT_DISCOUNT_VALUE_VAT']=$arResult['PRICES']['BASE']['PRINT_DISCOUNT_VALUE'];
$partnerId = isPartnerClient();
$arResult['PARTNER_ID'] = $partnerId;
if($partnerId){
	$arPrice = CCatalogIBlockParameters::getPriceTypesList();
    if(isset($arPrice['purchasePrice_'.$partnerId])){
        $arResult['MIN_PRICE_EXTRA']['VALUE']=$arResult['PRICES']['purchasePrice_'.$partnerId]['VALUE'];
        $arResult['MIN_PRICE_EXTRA']['PRINT_DISCOUNT_VALUE_VAT']=$arResult['PRICES']['purchasePrice_'.$partnerId]['PRINT_VALUE'];
    }
    if(isset($arPrice['retailPrice_'.$partnerId])){
        $arResult['MIN_PRICE_RETAIL']['VALUE']=$arResult['PRICES']['retailPrice_'.$partnerId]['VALUE'];
        $arResult['MIN_PRICE_RETAIL']['PRINT_DISCOUNT_VALUE_VAT']=$arResult['PRICES']['retailPrice_'.$partnerId]['PRINT_VALUE'];
    }
}/*elseif(count($_SESSION["SESS_AUTH"]["GROUPS"])){
    $key_extra_price='';
    $key_extra_price_okr='';
    foreach($arResult['PRICES'] as $price_key=>$price_array){
    ...
    if(!empty($key_extra_price)){
        $arResult['MIN_PRICE_EXTRA']['VALUE']=$arResult['PRICES'][$key_extra_price]['VALUE'];
        $arResult['MIN_PRICE_EXTRA']['PRINT_DISCOUNT_VALUE_VAT']=$arResult['PRICES'][$key_extra_price]['PRINT_DISCOUNT_VALUE'];
    }
}*/
?>
