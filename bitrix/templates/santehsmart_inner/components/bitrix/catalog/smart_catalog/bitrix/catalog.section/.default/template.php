...
foreach ($arResult['ITEMS'] as $key => $arItem)
{
   ...
   title="<? echo $strTitle; ?>"><?
    if ('Y' == $arParams['SHOW_DISCOUNT_PERCENT'] && !$arResult['PARTNER_ID'])
    {
        ?>
...
////////////////////////////////////////////////////////////////////
        ...
        $deliveryCost=$logIndex[$resSec['UF_LOGISTYC_INDEX']];
        $deliveryFreeCount=true;
        }
        if (($deliveryCost<=$freeDelFrom = $arItem['PROPERTIES']['morz']['VALUE'] / 6) && !$arResult['PARTNER_ID']) {
            echo "<img style='position: absolute; bottom:50px; left:-30px' src='/images/freeDelivery.png'>";
        }
...
?>
        <?////////////////////OLDPRICE
        if(($arItem['PROPERTIES']['OLD_PRICE']['VALUE']>1) && !$arResult['PARTNER_ID']):?>
            <div class="sale_marker">Распродажа</div>
            <div class="sale_old_price"><?=$arItem['PROPERTIES']['OLD_PRICE']['VALUE']?> руб.</div>
        <?endif?>
...
<div class="flyPageFragment">
  <table>
    <tr>
      ...
      <td class="rrc_price"><?=$arItem['MIN_PRICE_RETAIL']['PRINT_DISCOUNT_VALUE_VAT']?></td>
    </tr>
    <tr>
      <td class="fragmentTitle">МОЦ:</td>
      <td class="mop_price<?=($arItem['NALICH'])?'':' not-avail'?>" id="<? echo $arItemIDs['PRICE']; ?>"><?=$arItem['MIN_PRICE_EXTRA']['PRINT_DISCOUNT_VALUE_VAT']?></td>
    </tr>
    <tr>
      <td class="stock<?=($arItem['NALICH'])?'':' not-avail'?>" colspan="2"><div class="stockVal"><?=$arItem['CATALOG_QUANTITY']?> шт
          <div class="bx_catalog_item_controls_blockone" style="display: inline-block;margin-top: -2px;"><div style="display: inline-block;position: relative;">
      ...
      <tr>
          <td class="basketButton" colspan="2">
              <button id="<? echo $arItemIDs['BUY_LINK']; ?>" class="bx_bt_button bx_medium toOrder active bb<?=$arItem['XML_ID']?>"><?=($arItem['NALICH'])?'Добавить в заявку':'Уточнить'?></button>
          </td>
      </tr>
  </table>
</div>
...
<div class="bx_catalog_item_price"><div id="<? echo $arItemIDs['PRICE']; ?>" class="bx_price"><?
        if (!empty($arItem['MIN_PRICE']))
        {
            ...
            {
                ?> <span><? echo $arItem['PRICES']['BASE']['PRINT_DISCOUNT_VALUE_VAT']; ?></span><?
            }
        }
        ?></div>
    ...
    $quantCategory = 0;
    if($arItem['CATALOG_QUANTITY'] > $qLog[$UF_LOGISTYC_INDEX])
        $quantCategory = 3;//Достаточно
    elseif($arItem['CATALOG_QUANTITY'] > 1)
    ...
    <div class="nonebx_catalog_quantity" style="color:<?=$qCol[$quantCategory]?>">
        <?
        if($quantCategory == 3){?>
            <span style="color:<?=$qCol[3]?>">В наличии:</span><br>достаточно<?if($boolAdminPartner){?> - <?=$arItem['CATALOG_QUANTITY']?> шт.<?}?>
        <?}elseif($quantCategory == 2){?>
            <span style="color:<?=$qCol[3]?>">В наличии:</span><br>мало<?if($boolAdminPartner){?> - <?=$arItem['CATALOG_QUANTITY']?> шт.<?}?>
    ...
    <div class="nonebx_catalog_buy">
        <?
        if ($arItem['CAN_BUY'])
        {?>
                <!--<a class="bx_big bx_bt_button_type_2 bx_cart" href="?action=ADD_TO_COMPARE_LIST&id=<?=$arItem['ID']?>">
        ...
        {
            ?><div class="bx_catalog_item_controls_blockone"><a class="bx_bt_button_type_2 bx_medium" onclick="window.setProductNameSku('<?=$arItem['NAME']?>','<?=$arItem['XML_ID']?>')"><?
...
if (!isset($arItem['OFFERS']) || empty($arItem['OFFERS']))
{
    ...
    <div class="bx_catalog_item_controls"><?/*
        ...
        ?>
        <?
        $boolBaseShow=$_COOKIE['showSpecPrice']=='1';
        ?>
        <div class="bx_base_price" id="<? echo $arItemIDs['BASE_PRICE']; ?>" style="display: <? echo ($boolBaseShow ? '' : 'none'); ?>"><?=empty($arItem['PRICES']['SPEC']['PRINT_DISCOUNT_VALUE_VAT'])?'':(GetMessage("CT_SPEC_PRICE").': '.$arItem['PRICES']['SPEC']['PRINT_DISCOUNT_VALUE_VAT']); ?></div>
...
<span class="bitrix_description"><?=$arResult['DESCRIPTION']?></span>
<?endif?>
