...
function checkOut()
{
	...
	$('#basket_items>tbody>tr').each(function(){
		...
		var count=$(this).find('.custom .centered input').val();
		var name=$(this).find('.bx_ordercart_itemtitle a').text();
		cart_content[sku]=count;
		cart_names[sku]=name;
	});

	$.post( "/bitrix/templates/santehsmart_inner/components/bitrix/catalog/smart_catalog/bitrix/catalog.section/.default"
		+ "/ajax.php", {"cart_content":cart_content,"cart_names":cart_names} ,function( msg ) {//*******INTEGRATE NAMES!!!!!!!!!!!
		if(msg.status == 'ok'){
